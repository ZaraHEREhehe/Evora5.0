...
public class PomodoroView {
    ...
    private void initializeComponents() {
        ...
        // Happiness Label
        happinessLabel = new Label("😊 Happiness: 85%");
        happinessLabel.setStyle("-fx-text-fill: " + Pastel.FOREST + "; -fx-font-size: 14px; -fx-font-weight: bold; " +
                "-fx-background-color: " + Pastel.IVORY + "; -fx-background-radius: 10; " +
                "-fx-border-radius: 10; -fx-border-color: " + Pastel.DUSTY_PINK + "; " +
    ...
    private void setupStyles() {
        ...
        // Set slider styles
        String sliderStyle = "-fx-control-inner-background: " + Pastel.IVORY + "; " +
                "-fx-background-color: " + Pastel.DUSTY_PINK + "; " +
        ...
        for(int i = 0; i < Math.max(4, sessions); i++) {
            ...
            if(i < sessions) {
                dot.setFill(new LinearGradient(0,0,1,0,true,CycleMethod.NO_CYCLE,
                        new Stop(0, Color.web(Pastel.PINK)), new Stop(1, Color.web(Pastel.PURPLE))));
            } else {
    ...
    public void updateButtonState(boolean isRunning, boolean isBreak) {
        ...
        // Update reset button style based on break state
        resetButton.setStyle(getSecondaryButtonStyle());
    }
    ...
    // Getters for UI components that need event handlers
    ...
    private void updateCircleGradient(boolean isBreak) {
        if (isBreak) {
            timerCircle.setStroke(new LinearGradient(0, 0, 1, 0, true, CycleMethod.NO_CYCLE,
                    new Stop(0, Color.web(Pastel.BLUE)), new Stop(1, Color.web(Pastel.LAVENDER))));
        } else {
            timerCircle.setStroke(new LinearGradient(0, 0, 1, 0, true, CycleMethod.NO_CYCLE,
                    new Stop(0, Color.web(Pastel.PINK)), new Stop(1, Color.web(Pastel.PURPLE))));
        }
